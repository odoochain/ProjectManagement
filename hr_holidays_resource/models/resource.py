...
class ResourceCalendar(models.Model):
    ...
    @api.model
    def _update_contract_time_off(self):
        """
        ...
        contract_time_off_holiday_status_id = self.env.ref("hr_holidays_resource.holiday_status_cto",
                                                           raise_if_not_found=False)
        contract_time_off_partial_holiday_status_id = self.env.ref("hr_holidays_resource.holiday_status_ctop",
                                                                   raise_if_not_found=False)
        for resource_calendar in resource_calendars:
            non_working_periods = {}
            for day in ["0", "1", "2", "3", "4"]:
                working_periods = resource_calendar.attendance_ids.filtered(lambda r: r.dayofweek == day)
                ...
                if not working_periods.filtered(lambda wp: wp.day_period == "morning"):
                    ...
                    continue
                if not working_periods.filtered(lambda wp: wp.day_period == "afternoon"):
            ...
            if non_working_periods:
                ...
                for day,period in non_working_periods.items():
                    all_non_working_dates = find_all_dates_for_day_of_week_between_range(today,
                                                                                         three_months_from_today, day)
                    if period == "full day":
                        if contract_time_off_holiday_status_id:
                            for resource in resource_obj.search(
                                    [("calendar_id", "=", resource_calendar.id), ("resource_type", "=", "user")]):
                                for non_working_date in all_non_working_dates:
                                    try:
                                        with self.env.cr.savepoint():
                                            hr_leave_obj.create(
                                                self.prepare_hr_leave_vals(non_working_date,
                                    ...
                                    try:
                                        with self.env.cr.savepoint():
                                            hr_leave_obj.create(
                                                self.prepare_hr_leave_vals(non_working_date,
                                                                           contract_time_off_holiday_status_id,
                                                                           resource, attendances, period))
                                    except Exception:
                                        continue

    def prepare_hr_leave_vals(self, date, holiday_status_id, resource, attendances, period=False):
        ...
        if period:
            ...
            attendance_from = next((att for att in attendances if att.day_period == period),
                                   attendances[0] if attendances else default_value)
            # find last attendance coming before last_day for the specified period
            attendance_to = next((att for att in reversed(attendances) if att.day_period == period),
                                 attendances[-1] if attendances else default_value)
        else:
            # find first attendance coming after first_day
            attendance_from = next((att for att in attendances if int(att.dayofweek) >= date.weekday()),
                                   attendances[0] if attendances else default_value)
            # find last attendance coming before last_day
            attendance_to = next((att for att in reversed(attendances) if int(att.dayofweek) <= date.weekday()),
                                 attendances[-1] if attendances else default_value)
        ...
        vals["date_from"] = self.env["hr.leave"]._get_start_or_end_from_attendance(hour_from, date.date(), employee)
        vals["date_to"] = self.env["hr.leave"]._get_start_or_end_from_attendance(hour_to, date.date(), employee)
        vals["request_date_from"], vals["request_date_to"] = vals["date_from"].date(), vals["date_to"].date()
        if period:
            vals.update({"number_of_days": 0.5, "request_date_from_period": "am" if period == "morning" else "pm",
                         "request_unit_half": True, "request_date_from": vals["date_from"],
    ...
    def _get_attendances(self, resource_calendar_id):
        domain = [("calendar_id", "=", resource_calendar_id), ("display_type", "=", False)]
